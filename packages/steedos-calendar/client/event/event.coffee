...
Template.calendarContainer.onRendered ->
	...
	if Steedos.isAndroidOrIOS()
		...
		$("#calendar").on("touchstart", (event, options)->
			...
			isSwiping = true
			$(contentWrapperSelector).removeClass("transform-fix")
		...
		$("#calendar").on("touchend", (event, options)->
			...
			if (startOffsetX >= minSplitPx || startOffsetX <= -minSplitPx) and startOffsetY < maxOffsetY and startOffsetY > -maxOffsetY
			...
			startOffsetX = 0
			startOffsetY = 0
			$(contentWrapperSelector).addClass("transform-fix")
		...
		$("#calendar").on("touchmove", (event, options)->
			...
			startOffsetY = event.originalEvent.touches[0].clientY - startY
			if startOffsetX < minSplitPx and startOffsetX > -minSplitPx
				return
			if startOffsetY < maxOffsetY and startOffsetY > -maxOffsetY
				translate = startOffsetX
				if translate > maxSplitPx
					translate = maxSplitPx
				else if translate < -(maxSplitPx)
					translate = -(maxSplitPx)
				$(contentWrapperSelector).css("transform","translate(#{translate}px, 0)")
